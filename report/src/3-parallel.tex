...
\subsection{Merge}
This phase, the reduce phase, begins with a barrier: after this point all threads have populated their own local hash table and they can start to merge the local hash table in the global one.
\begin{itemize}
    ...
    \item \textbf{Zero-Copy Node Relinking}: When merging a local bucket into a global one, if an n-gram is found we sum the counters, if it's not found in the global table we simply relink the existing node pointer from the local table to the global table; this avoid a several call to \texttt{malloc}. At the end of the parallel region, we use the \texttt{arena\_connect} function to append the local memory arenas (containing the relinked nodes) to the global one.
    ...
    \item \textbf{False Sharing}: With the usage of \texttt{dinamyc} we could have a potential performance killer with the false sharing: since multiple pointers (8 bytes each) fit in a single cache line (typically 64 bytes), adjacent buckets share the same cache line; by default we have $\texttt{block\_size}=1$, so if two threads process adjacent buckets, they would continuously invalidate each other's cache lines. So we have defined the block size as $(64/\texttt{sizeof(Node*)})\times16$: each chunk is aligned with cache lines; the chunk is large enough (16 cache lines, 1024 bytes) to amortize the OpenMP scheduling overhead. This empirical dimension must be most big if during the profiling we see that the prpgram spend to much time in \texttt{malloc} function.
    \item \textbf{Implicit Barrier}: At the end of a \texttt{\#pragma omp for} there is an implicit barrier. Insert a \texttt{nowait} cluase would be a mistake, becasue it implies that the local table of the other thread will be free cusing a segmentation fault.
\end{itemize}
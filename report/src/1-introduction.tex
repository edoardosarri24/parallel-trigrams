...
The code works well, for simplifying the implementation, only with English sentences. This allows us to treat a character as a single byte.

We use a simple and small plain text file to understand the correctness of the program. This file came from the \href{https://wortschatz.uni-leipzig.de/en/download/eng}{Wortschatz Leipzig project}, that contains a big corpus of English text; in specific \href{https://downloads.wortschatz-leipzig.de/corpora/eng_news_2024_1M.tar.gz}{this link} is the direct download of the file. For the final test and for the analysis we take a plain text from \href{https://huggingface.co/datasets/allenai/c4/tree/main/en}{Hugging Face}; we download the file until its dimension reaches 1GB.
...
In fact, suppose that $H_i$ can be stored in 32 bits we can prove that the intermediate variable used can be stored in 32 bits: in the worst case we have that $intermediate\_var\_dim=(M-1)\cdot p+c_{max}<2^{32}$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Execution}
The execution of the project requires two steps: the download of test input or real input (the one used for the analysis), simply executing \texttt{download\_test.sh} script or \texttt{download\_input.sh} script in \texttt{exec/} folder; the second step is executing the script named \texttt{execution.sh}, always in the \texttt{exec/} folder. This last script execution executes the program on the file previously downloaded input text with all performance flags on. In the same folder there are also others script for the execution with profiling (different type) or with the sanitizers or in debug mode.


To execute the project you have to execute one of the shell script in \href{https://github.com/edoardosarri24/parallel-trigrams/blob/master/exec}{exec folder}. The \textit{download\_data.sh} is a helper script that is used in the other.